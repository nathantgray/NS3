...
macro(
  ...
  if(${folder} MATCHES "src")
    set(module_header ${libname})
  else()
    set(module_header contrib-${libname})
  endif()
  ...
  if(${NS3_PYTHON_BINDINGS} AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/bindings")
    ...
    string(REPLACE "-" "_" libname_sub input) # * causes problems (e.g. csma-layout), replace with _ (e.g. csma_layout)
    ...
    if((NOT EXISTS ${module_hdr}) OR (NOT EXISTS ${module_src})) # OR TRUE) # to force reprocessing
      ...
      execute_process(
        COMMAND ${CMAKE_COMMAND} -E env PYTHONPATH=${CMAKE_OUTPUT_DIRECTORY} ${modulegen_modular_command}
                ${CMAKE_CURRENT_SOURCE_DIR} ${arch} ${libname_sub} ${module_src}
        TIMEOUT 60
    ...
    set_target_properties(
      ${libname}-bindings
      PROPERTIES OUTPUT_NAME ${prefix}${libname}
                 PREFIX ""
...
macro(
  ...
  file(COPY ${files_to_copy} DESTINATION ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${folder}/${libname}/examples/)

  process_tests(
    "${ignore_example}" "${name}" "${examples}" "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${folder}/${libname}/examples/"
  )
  unset(examples)
endmacro()
