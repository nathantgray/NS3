...
macro(process_options)
  ...
  if(${NS3_PYTHON_BINDINGS} AND Python3::Development)
...
# Import macros used to extract parameters from examples_to_run.py
include(buildsupport/custom_modules/ns3_extract_examples_to_run_arguments.cmake)

# Import macros to configure ctests with parameters extracted from examples_to_run.py
include(buildsupport/custom_modules/ns3_cmake_tests.cmake)
...
macro(build_lib_example name source_files header_files libraries_to_link files_to_copy)
  build_lib_example_impl("src" "${name}" "${source_files}" "${header_files}" "${libraries_to_link}" "${files_to_copy}")
endmacro()
...
macro(build_example name source_files header_files libraries_to_link)
  ...
  process_tests(
    "${ignore_example}" "${name}" "${examples}" "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/examples/${examplefolder}/"
  )
  unset(examples)
endmacro()
